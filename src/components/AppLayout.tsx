...
// v2.0 - The complete, integrated, and polished application layout component.
...
import { MessageSquare, Users, Plus, Settings, Sun, Moon, Menu, Sparkles, MoreHorizontal, Edit, FilePenLine, Trash2 } from 'lucide-react';
...
const ChatModeSidebar = (props: ChatModeSidebarProps) => (
    <div className="flex flex-col h-full">
        <div className="p-2 flex-shrink-0">
            ...
            <Select value={props.selectedTeam?.teamId || ''} onValueChange={props.onSelectTeam}>
        ...
        <div className="flex-grow overflow-y-auto mt-4 pt-4 border-t border-gray-200 dark:border-zinc-800">
            ...
            <nav className="mt-2 space-y-1 px-2">
                {props.chatHistory.length > 0 ? (props.chatHistory.map(chat => (
...
interface TeamModeSidebarProps {
    teams: Team[];
...
const TeamModeSidebar = (props: TeamModeSidebarProps) => (
    <div className="flex flex-col h-full">
        <div className="p-2 flex-shrink-0">
            <div className="text-center mb-4">
        ...
        <div className="flex-grow overflow-y-auto mt-4 pt-4 border-t border-gray-200 dark:border-zinc-800">
            ...
            <nav className="mt-2 space-y-1 px-2">{props.teams.map(team => (<Button key={team.teamId} variant="ghost" className={`w-full justify-start ${sidebarHoverStyle} ${props.activeTeam?.teamId === team.teamId ? sidebarSelectedStyle : ''}`} onClick={() => props.onTeamClick(team)} >{team.name}</Button>))}</nav>
        </div>
    </div>
);
      ...
      <aside className={`flex flex-col flex-shrink-0 bg-gray-100 dark:bg-[#141415] transition-all duration-300 ${isSidebarOpen ? 'w-72 p-2' : 'w-0 border-0 p-0'}`}>
        <div className="flex flex-col flex-grow overflow-hidden">
            <div className={`flex-shrink-0 p-2 transition-opacity duration-200 ${isSidebarOpen ? 'opacity-100' : 'opacity-0'}`}>
                <ToggleGroup type="single" value={props.activeMode} onValueChange={(value) => { if (value) props.setActiveMode(value as 'chat' | 'team'); }} className="w-full grid grid-cols-2"> 
            ...
            <div className={`flex-grow mt-2 overflow-y-auto transition-opacity duration-200 ${isSidebarOpen ? 'opacity-100' : 'opacity-0'}`}>
              {props.activeMode === 'chat' ? ( <ChatModeSidebar {...props} /> ) : ( <TeamModeSidebar {...props} onTeamClick={props.onSetActiveTeam} /> )}
            </div>
